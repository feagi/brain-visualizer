...
impl FeagiTypeValidator {
    ...
    pub fn validate_agent_compatibility(
        ...
        if cortical_type_bind.is_input() {
            if cortical_type_bind.is_cartesian_plane() {
                ...
                {
                    warnings.push(&GString::from(
                        "CartesianPlane encoding expects vision/camera input",
                    ));
                ...
                recommendations.push(&GString::from(
                    "Ensure agent provides data normalized to 0-100%",
                ));
            }
        }
        ...
        if cortical_type_bind.is_output() {
            if cortical_type_bind.is_percentage_encoding() {
                ...
                {
                    recommendations.push(&GString::from(
                        "Percentage encoding typically used for motors/servos",
                    ));
                }
                recommendations.push(&GString::from(
                    "Output will be percentage values (0-100%) - ensure actuators are calibrated",
                ));
            }
        }
    ...
    pub fn validate_template(
        ...
        if cortical_type_bind.get_internal_type().is_none() {
            let msg = GString::from(
                format!("Template '{}' has no cortical type defined", template_name).as_str(),
            );
            errors.push(&msg);
            result.set("valid", false);
        ...
        if category == "UNKNOWN" {
            let msg = GString::from(
                format!("Template '{}' has unknown cortical type", template_name).as_str(),
            );
            errors.push(&msg);
        }
    ...
    ///
    /// Returns: VarDictionary with complete validation info
    #[func]
