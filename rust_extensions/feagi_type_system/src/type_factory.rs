...
use feagi_structures::genomic::cortical_area::{
    CoreCorticalType, CorticalAreaType, CustomCorticalType, IOCorticalAreaConfigurationFlag,
    MemoryCorticalType,
};
use godot::prelude::*;
...
impl FeagiCorticalTypeFactory {
    ...
    #[func]
    pub fn create_ipu_signed_percentage(
        frame_handling: i32,
        positioning: i32,
    ) -> Gd<FeagiCorticalType> {
        let handling = Self::i32_to_frame_handling(frame_handling);
        let pos = Self::i32_to_positioning(positioning);
        Self::create_with_type(CorticalAreaType::BrainInput(
    ...
    #[func]
    pub fn create_opu_signed_percentage(
        frame_handling: i32,
        positioning: i32,
    ) -> Gd<FeagiCorticalType> {
        let handling = Self::i32_to_frame_handling(frame_handling);
    ...
    pub fn create_custom() -> Gd<FeagiCorticalType> {
        Self::create_with_type(CorticalAreaType::Custom(
            CustomCorticalType::LeakyIntegrateFire,
        ))
    }
    ...
    /// }
    #[func]
    pub fn from_api_dict(api_dict: VarDictionary) -> Gd<FeagiCorticalType> {
        // Extract fields
        ...
        let positioning = if let Ok(details_dict) = encoding_details.try_to::<VarDictionary>() {
        ...
        let cortical_type = match (category.as_str(), data_type.as_str()) {
            ...
            ("IPU", "Misc") | ("IPU", _) => {
                CorticalAreaType::BrainInput(IOCorticalAreaConfigurationFlag::Misc(frame_handling))
            }
            ...
            ("OPU", "Misc") | ("OPU", _) => {
                CorticalAreaType::BrainOutput(IOCorticalAreaConfigurationFlag::Misc(frame_handling))
            }
