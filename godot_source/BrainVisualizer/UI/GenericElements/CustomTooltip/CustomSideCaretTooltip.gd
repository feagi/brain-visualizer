...
class_name CustomSideCaretTooltip

## Horizontal tooltip: left-pointing caret + body to the right (avoids covering menus below the anchor).
const TOOLTIP_GAP_X: float = 10.0
const FADE_DURATION: float = 0.15
const TOOLTIP_PADDING: Vector2 = Vector2(12, 8)
const MAX_WRAP_WIDTH_BASE_PX: float = 280.0
const MIN_LABEL_WIDTH_BASE_PX: float = 40.0
const MAX_VISIBLE_TEXT_LINES: int = 2

## Caret span (horizontal thickness); triangle tip at x=0, base at x=width.
const TRIANGLE_WIDTH_BASE_PX: float = 12.0
...
func _ready() -> void:
	...
	_caret = TooltipCaretLeft.new()
	_caret.name = "TooltipCaretLeft"
	_caret.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_caret.size_flags_vertical = Control.SIZE_EXPAND_FILL
	add_child(_caret)
...
func _apply_caret_metrics() -> void:
	if _caret == null or not is_instance_valid(_caret):
		return
	var s := _ui_scale()
	var tw: float = TRIANGLE_WIDTH_BASE_PX * s
	_caret.custom_minimum_size = Vector2(tw, 0)
...
func show_tooltip(text: String, anchor_control: Control) -> void:
	...
	if anchor_control == null or not is_instance_valid(anchor_control):
		push_warning("CustomSideCaretTooltip: Anchor control is null or invalid")
		hide_tooltip()
		return
	if _label == null or not is_instance_valid(_label):
...
func _position_tooltip() -> void:
	...
	var x_pos: float = anchor_rect.position.x + anchor_rect.size.x + gap
	var y_pos: float = anchor_rect.position.y + (anchor_rect.size.y - tooltip_size.y) * 0.5
	if x_pos + tooltip_size.x > vr.position.x + vr.size.x - margin:
		x_pos = anchor_rect.position.x - tooltip_size.x - gap
	...
	if max_x >= min_x:
		...
		x_pos = min_x
	var min_y: float = vr.position.y + margin
	var max_y: float = vr.position.y + vr.size.y - tooltip_size.y - margin
	if max_y >= min_y:
...
## Left-pointing triangle: tip at left center, base on the right edge (meets panel).
class TooltipCaretLeft extends Control:
	...
	func _draw() -> void:
		...
		if w <= 0.0 or h <= 0.0:
			return
		var tip := Vector2(0.0, h * 0.5)
		var base_t := Vector2(w, h * 0.22)
		var base_b := Vector2(w, h * 0.78)
		var pts := PackedVector2Array([tip, base_t, base_b])
		draw_colored_polygon(pts, fill_color)
		draw_line(pts[0], pts[1], edge_color, 1.0, true)
		draw_line(pts[0], pts[2], edge_color, 1.0, true)
		draw_line(pts[1], pts[2], edge_color, 1.0, true)
