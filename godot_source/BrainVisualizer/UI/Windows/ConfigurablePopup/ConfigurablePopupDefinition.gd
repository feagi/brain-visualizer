...
func _init(window_title: StringName, window_message: StringName, window_buttons: Array[Button], window_minumum_size: Vector2i = Vector2i(0,0) ) -> void:
	title = window_title
...
static func create_close_button(text: StringName = "OK") -> Button:
	var button: Button = Button.new()
	button.text = text
	return button

## Creates a Button that does an action
static func create_action_button(action: Callable, text: StringName) -> Button:
	var button: Button = Button.new()
	button.text = text
...
static func create_single_button_close_popup(window_title: StringName, window_message: StringName, button_text: StringName = "OK", window_minumum_size: Vector2i = Vector2i(0,0)) -> ConfigurablePopupDefinition:
	var button: Button = ConfigurablePopupDefinition.create_close_button(button_text)
	var button_arr: Array[Button] = [button]
...
static func create_cancel_and_action_popup(window_title: StringName, window_message: StringName, accept_action: Callable, accept_text: StringName = "OK",  cancel_text: StringName = "Cancel", window_minumum_size: Vector2i = Vector2i(0,0)) -> ConfigurablePopupDefinition:
	var button_ok: Button = ConfigurablePopupDefinition.create_action_button(accept_action, accept_text)
	var button_close: Button = ConfigurablePopupDefinition.create_close_button(cancel_text)
	var button_arr: Array[Button] = [button_ok, button_close]
	return ConfigurablePopupDefinition.new(window_title, window_message, button_arr, window_minumum_size)
