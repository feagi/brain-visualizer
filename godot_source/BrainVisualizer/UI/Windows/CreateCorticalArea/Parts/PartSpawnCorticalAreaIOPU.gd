...
var _dropdown: TemplateDropDown
var _location: Vector3iSpinboxField
var _channel_count: SpinBox
var _iopu_image: TextureRect
var _current_dimensions_as_per_channel_count: Vector3i = Vector3i(0,0,0)
var _is_IPU_not_OPU: bool

func _ready() -> void:
	_dropdown = $HBoxContainer2/TopSection/TemplateDropDown
	_location = $HBoxContainer/Fields/Location
	_channel_count = $HBoxContainer/Fields/ChannelCount
...
func cortical_type_selected(cortical_type: AbstractCorticalArea.CORTICAL_AREA_TYPE, preview_close_signals: Array[Signal]) -> void:
	_dropdown.load_cortical_type_options(cortical_type)
	_is_IPU_not_OPU = cortical_type == AbstractCorticalArea.CORTICAL_AREA_TYPE.IPU
	var move_signals: Array[Signal] = [_location.user_updated_vector]
	var resize_signals: Array[Signal] = [calculated_dimensions_updated]
	BV.UI.start_cortical_area_preview(_location.current_vector, _current_dimensions_as_per_channel_count, move_signals, resize_signals, preview_close_signals)
	if _is_IPU_not_OPU:
...
func _drop_down_changed(cortical_template: CorticalTemplate) -> void:
	_current_dimensions_as_per_channel_count = cortical_template.calculate_IOPU_dimension(int(_channel_count.value))
