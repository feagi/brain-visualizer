...
const DEFAULT_PSP_MULTIPLIER: int = 1
...
var post_synaptic_current_multiplier: int:
...
var _post_synaptic_current_multiplier: int
...
static func from_FEAGI_JSON(mapping_property: Dictionary) -> SingleMappingDefinition:
	...
	# Accept older float payloads but coerce deterministically to int for internal consistency.
	var psp_multiplier: int = int(mapping_property["postSynapticCurrent_multiplier"])
...
## Create Object
func _init(morphology: BaseMorphology, positive_scalar: Vector3i, psp_multilpier: int, plasticity: bool, 
...
func to_FEAGI_JSON() -> Dictionary:
	...
	if !_plasticity_flag and !associative_memory:
		return {
			...
			# Must be an integer for FEAGI's strict parsing on the API side.
			"postSynapticCurrent_multiplier": int(_post_synaptic_current_multiplier),
