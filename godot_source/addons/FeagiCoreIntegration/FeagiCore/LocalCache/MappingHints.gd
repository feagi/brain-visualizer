...
const MORPHOLOGY_RESTRICTIONS: Dictionary = {
	AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN: {
		AbstractCorticalArea.CORTICAL_AREA_TYPE.MEMORY: [&"memory"]
		},
	AbstractCorticalArea.CORTICAL_AREA_TYPE.MEMORY: {
		AbstractCorticalArea.CORTICAL_AREA_TYPE.MEMORY:[&"memory"],
		AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN: [&"projector"]
	}
}
...
const MAPPING_COUNT_LIMITS: Dictionary = {
	AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN: {
		AbstractCorticalArea.CORTICAL_AREA_TYPE.MEMORY: 1
		},
	AbstractCorticalArea.CORTICAL_AREA_TYPE.MEMORY: {
		AbstractCorticalArea.CORTICAL_AREA_TYPE.MEMORY: 1,
		AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN: 1
	}
}
...
const MAPPING_CORTICAL_TYPE_SPECIAL_CASES: Dictionary = {
	AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN: {
		AbstractCorticalArea.CORTICAL_AREA_TYPE.MEMORY: [MAPPING_SPECIAL_CASES.ANY_TO_MEMORY]
		},
	AbstractCorticalArea.CORTICAL_AREA_TYPE.MEMORY: {
		AbstractCorticalArea.CORTICAL_AREA_TYPE.MEMORY: [MAPPING_SPECIAL_CASES.ANY_TO_MEMORY, 
			MAPPING_SPECIAL_CASES.MEMORY_TO_ANY, 
			MAPPING_SPECIAL_CASES.MEMORY_TO_MEMORY],
		AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN: [MAPPING_SPECIAL_CASES.MEMORY_TO_ANY]
	}
}
...
static func get_allowed_morphologies_to_map_toward(source_cortical_area: AbstractCorticalArea, destination_cortical_area: AbstractCorticalArea) -> Array[BaseMorphology]:
	...
	if source_type in MORPHOLOGY_RESTRICTIONS.keys():
		...
		if destination_type in MORPHOLOGY_RESTRICTIONS[source_type]:
			...
			acceptable_morphologies_str.assign(MORPHOLOGY_RESTRICTIONS[source_type][AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN])
			return  FeagiCore.feagi_local_cache.morphologies.attempt_to_get_morphology_arr_from_string_name_arr(acceptable_morphologies_str)
	else:
		# Source type has no specific mapping
		if destination_type in MORPHOLOGY_RESTRICTIONS[AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN]:
			# Destination does have a restriction
			acceptable_morphologies_str.assign(MORPHOLOGY_RESTRICTIONS[AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN][destination_type])
			return  FeagiCore.feagi_local_cache.morphologies.attempt_to_get_morphology_arr_from_string_name_arr(acceptable_morphologies_str)
		else:
...
static func get_default_morphology_to_map_with(source_cortical_area: AbstractCorticalArea, destination_cortical_area: AbstractCorticalArea) -> BaseMorphology:
	...
	if source_type in MORPHOLOGY_DEFAULTS.keys():
		...
		if destination_type in MORPHOLOGY_DEFAULTS[source_type]:
			return  FeagiCore.feagi_local_cache.morphologies.available_morphologies[MORPHOLOGY_DEFAULTS[source_type][destination_type]]
		else:
			return  FeagiCore.feagi_local_cache.morphologies.available_morphologies[MORPHOLOGY_DEFAULTS[source_type][AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN]]
	else:
		# Source type has no specific mapping
		if destination_type in MORPHOLOGY_RESTRICTIONS[AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN]:
			# Destination does have a restriction
			return  FeagiCore.feagi_local_cache.morphologies.available_morphologies[MORPHOLOGY_DEFAULTS[AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN][destination_type]]
		else:
			# No mapping restriction found at all
			return  FeagiCore.feagi_local_cache.morphologies.available_morphologies[MORPHOLOGY_DEFAULTS[AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN][AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN]]
...
static func get_allowed_mapping_count(source_cortical_area: AbstractCorticalArea, destination_cortical_area: AbstractCorticalArea) -> int:
	...
	if source_type in MAPPING_COUNT_LIMITS.keys():
		...
		if destination_type in MAPPING_COUNT_LIMITS[source_type]:
			...
			return MAPPING_COUNT_LIMITS[source_type][AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN]
	else:
		# Source type has no specific mapping
		if destination_type in MAPPING_COUNT_LIMITS[AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN]:
			# Destination does have a restriction
			return  MAPPING_COUNT_LIMITS[AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN][destination_type]
		else:
			# No mapping restriction found at all
			return -1
...
static func get_special_cases_for_mapping_to_destination(source_cortical_area: AbstractCorticalArea, destination_cortical_area: AbstractCorticalArea) -> Array[MAPPING_SPECIAL_CASES]:
	...
	if source_type in MAPPING_CORTICAL_TYPE_SPECIAL_CASES.keys():
		...
		if destination_type in MAPPING_CORTICAL_TYPE_SPECIAL_CASES[source_type]:
			...
			output.assign(MAPPING_CORTICAL_TYPE_SPECIAL_CASES[source_type][AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN])
			return output
	else:
		# Source type has no specific mapping
		if destination_type in MAPPING_CORTICAL_TYPE_SPECIAL_CASES[AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN]:
			# Destination does have a restriction
			output.assign(MAPPING_CORTICAL_TYPE_SPECIAL_CASES[AbstractCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN][destination_type])
			return  output
		else:
			# No mapping restriction found at all
			return output
...
static func is_mapping_property_array_invalid_for_cortical_areas(source_cortical_area: AbstractCorticalArea, destination_cortical_area: AbstractCorticalArea, mapping_properties: Array[MappingProperty]) -> bool:
...
func _init(source_cortical_area: AbstractCorticalArea, destination_cortical_area: AbstractCorticalArea) -> void:
