...
static func get_default_morphology_to_map_with(source_cortical_area: BaseCorticalArea, destination_cortical_area: BaseCorticalArea) -> BaseMorphology:
	...
	if source_type in MORPHOLOGY_DEFAULTS.keys():
		...
		if destination_type in MORPHOLOGY_DEFAULTS[source_type]:
			...
			return  FeagiCore.feagi_local_cache.morphologies.available_morphologies[MORPHOLOGY_DEFAULTS[source_type][BaseCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN]]
	else:
		# Source type has no specific mapping
		if destination_type in MORPHOLOGY_RESTRICTIONS[BaseCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN]:
			# Destination does have a restriction
			return  FeagiCore.feagi_local_cache.morphologies.available_morphologies[MORPHOLOGY_DEFAULTS[BaseCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN][destination_type]]
		else:
			# No mapping restriction found at all
			return  FeagiCore.feagi_local_cache.morphologies.available_morphologies[MORPHOLOGY_DEFAULTS[BaseCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN][BaseCorticalArea.CORTICAL_AREA_TYPE.UNKNOWN]]
