...
static func create_from_template(ID: StringName, template: CorticalTemplate, new_channel_count: int, parent_region: BrainRegion, visiblity: bool = true) -> OPUCorticalArea:
	return OPUCorticalArea.new(ID, template.cortical_name, template.calculate_IOPU_dimension(new_channel_count), parent_region, visiblity)
