...
func _init(gap_between_nodes: Vector2i, node_size: Vector2i)  -> void:
    ...
    for i in CorticalArea.CORTICAL_AREA_TYPE.keys():
        _ordered_cortical_area_counter_by_type.append(0) # This is Stupid. Too bad!
    
func add_cortical_area_to_memory_and_return_position(cortical_area_type: CorticalArea.CORTICAL_AREA_TYPE) -> Vector2i:
